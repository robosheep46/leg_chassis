...
static void InitQuaternion(float *init_q4)
{
    ...
    float angle = acosf(Dot3d(acc_init, gravity_norm));//acosf() 计算反余弦值，将余弦值转换为弧度制的夹角：
...
attitude_t *imu_init(void)
{
    attitude_t *attitude_instance =(attitude_t *)malloc(sizeof(attitude_t));
    imu_data = attitude_instance;
    if (!INS.init)
        INS.init = 1;
    else
    ...
    INS.AccelLPF = 0.0085;
    dwt_get_delta_time(&INS_DWT_Count);
    return attitude_instance;
}
