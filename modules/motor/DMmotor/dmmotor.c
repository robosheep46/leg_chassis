...
void dmmotor_set_control_mode(DMMotorInstance *motor,dm_motor_control_mode_e mode)
{
    if(motor->control_mode != mode)
    {
        ...
        uint8_t can_id_l = motor->motor_can_instace->tx_id & 0xFF;       // 低 8 位
        uint8_t can_id_h = (motor->motor_can_instace->tx_id >> 8) & 0x07; // 高 3 位
        
        uint8_t data[8] = {can_id_l, can_id_h,0x55, 10, mode, 0, 0, 0};
