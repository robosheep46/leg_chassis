...
int pos2tranXYZ(float *Xpt, float *Tpt, int numX , int numT)
{
  ...
  // first row
  Tpt[0 * 4 + 0] = arm_cos_f32(Xpt[4])*arm_cos_f32(Xpt[5]);
  Tpt[0 * 4 + 1] = -arm_cos_f32(Xpt[4])*arm_sin_f32(Xpt[5]);
  Tpt[0 * 4 + 2] = arm_sin_f32(Xpt[4]);
  Tpt[0 * 4 + 3] = Xpt[0];
  // second row
  Tpt[1 * 4 + 0] = arm_sin_f32(Xpt[3]) * arm_sin_f32(Xpt[4]) * arm_cos_f32(Xpt[5]) + arm_cos_f32(Xpt[3]) * arm_sin_f32(Xpt[5]);
  Tpt[1 * 4 + 1] = -arm_sin_f32(Xpt[3]) * arm_sin_f32(Xpt[4]) * arm_sin_f32(Xpt[5]) + arm_cos_f32(Xpt[3]) * arm_cos_f32(Xpt[5]);
  Tpt[1 * 4 + 2] = -arm_sin_f32(Xpt[3]) * arm_cos_f32(Xpt[4]);
  Tpt[1 * 4 + 3] = Xpt[1];
  // third row
  Tpt[2 * 4 + 0] = -arm_cos_f32(Xpt[3])*arm_sin_f32(Xpt[4])*arm_cos_f32(Xpt[5]) + arm_sin_f32(Xpt[3])*arm_sin_f32(Xpt[5]);
  Tpt[2 * 4 + 1] = arm_cos_f32(Xpt[3])*arm_sin_f32(Xpt[4])*arm_sin_f32(Xpt[5]) + arm_sin_f32(Xpt[3])*arm_cos_f32(Xpt[5]);
  Tpt[2 * 4 + 2] = arm_cos_f32(Xpt[3])*arm_cos_f32(Xpt[4]);
  Tpt[2 * 4 + 3] = Xpt[2];
  // forth row
  Tpt[3 * 4 + 0] = 0.0;
  Tpt[3 * 4 + 1] = 0.0;
